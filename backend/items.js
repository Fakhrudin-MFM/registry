    ...
    if (typeof data.hasOwnProperty === 'undefined' || data.hasOwnProperty(nm)) {
      ...
      if (pm) {
        if (pm.type === PropertyTypes.DATETIME) {
          ...
          if (!valid)
            throw new Error(`Invalid schedule value passed to attribute ${cm.getCaption()}.${pm.caption}`);
          else
            result[nm] = dt;
...
function applyDetails(metaRepo, dataRepo, master, collection, details, changeLog, operation, user) {
  return function() {
    ...
    if (!p || p.getType() !== PropertyTypes.COLLECTION)
      throw new Error(`No ${collection} collection attribute found`);
    ...
    if (!dc)
      throw new Error(`No ${collection} collection item class found`);
        ...
        if (validateConditions) {
          ...
          if (typeof checker === 'function') {
            ...
            worker = itemGetter
              ...
              .then((result) => {
                if (!result)
                  throw new Error('The saved object does not satisfy the conditions for selecting valid values!');
    ...
    return checker.state(itemId)
      ...
      .then(state => new Promise((resolve, reject) => {
        ...
        auth.userProfile(state.user, (profile) => {
          if (!profile)
            return reject(new Error('lock owner not found'));
...
function vmEagerLoading(vm, cm) {
  ...
  for (let i = 0; i < vm.tabs.length; i++) {
    const tab = vm.tabs[i];
    tab.fullFields = tab.fullFields ? tab.fullFields : []; // In order not to fall, when there are no tabs in the meta
    tab.shortFields = tab.shortFields ? tab.shortFields : []; // In order not to fall, when there are no tabs in the meta
