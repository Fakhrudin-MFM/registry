...
(function () {
  ...
  function filterSelection(filters, $container, selection) {
    $container.empty();
    $('<option/>').text(__('Выберите фильтр...')).val(null).appendTo($container);
    if (filters) {
  ...
  ListFilter.prototype = {
    ...
    createCondition: function ($container, attrs, selection, options) {
      ...
      $('<option/>').text(__('Выберите фильтр...')).val(null).appendTo($selectAttr);
      if (attrs && attrs.length) {
  ...
  $.extend(HeaderFilter.prototype, {
    ...
    createHeader: function (index, cell) {
      ...
      if ($input) {
        $cell.append('<span class="dt-head-filter-toggle glyphicon glyphicon-filter" title="' + __('Фильтр') + '"></span>');
      ...
      if (params) {
        ...
        switch (type) {
          ...
          case NUMBER_TYPE:
          {
            var opers = {
              "=": __('= равно'),
              "&lt;": '&lt; ' + (type === DATE_TYPE ? __('ранее') : __('меньше')),
              "&gt;": '&gt; ' + (type === DATE_TYPE ? __('позднее') : __('больше')),
              "&lt;=": '&#x2264 ' + (type === DATE_TYPE ? __('ранее') : __('меньше')) + __(' или равно'),
              "&gt;=": '&#x2265 ' + (type === DATE_TYPE ? __('позднее') : __('больше')) + __(' или равно')
            };
            var content = '';
