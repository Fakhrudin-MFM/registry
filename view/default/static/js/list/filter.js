...
(function () {
  ...
  function filterSelection(filters, $container, selection) {
    $container.empty();
    $('<option/>').text('Select filter...').val(null).appendTo($container);
    if (filters) {
  ...
  ListFilter.prototype = {
    ...
    createCondition: function ($container, attrs, selection, options) {
      ...
      $('<option/>').text('Select field...').val(null).appendTo($selectAttr);
      if (attrs && attrs.length) {
  ...
  $.extend(HeaderFilter.prototype, {
    ...
    createHeader: function (index, cell) {
      ...
      if ($input) {
        $cell.append('<span class="dt-head-filter-toggle glyphicon glyphicon-filter" title="Filters"></span>');
