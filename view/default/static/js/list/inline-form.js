...
(function () {
  ...
  $.extend(RowForm.prototype, {
    ...
    remove: function () {
      var data = this.dt.row(this.row).data() || {},
        message = __('Убрать объект из коллекции?'),
      ...
      if (data.__class && data._id && confirm(message)) {
        if (_list.options.url && _list.options.url.remove) {
          $.post(_list.options.url.remove, {
            ...
            .fail(function (xhr) {
              messageCallout.error(xhr.$message || __('Ошибка при извлечении объектов'));
              console.error(xhr);
            })
    ...
    delete: function () {
      this.owner.list.del(__('Удалить объект?'), this.owner.list.toDelete('.'+ SELECTED_CLASS));
    },
    ...
    createWorkflowParams: function (d) {
      return 'data-id="'+ d.cls +'.'+ d.key +'" '
        ...
        + (d.data.confirmMessage || d.data.confirm
          ? 'data-confirm-message="' + (d.data.confirmMessage ? d.data.confirmMessage : __('Вы уверены?')) + '" '
          : '');
    },
